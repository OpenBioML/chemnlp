...
def process_docs(docs):
    valid = list(map(_get_smile_string, docs))
    valid = [doc for doc in valid if doc is not None]
    return random.choice(valid, len(valid)).tolist()


def _get_smile_string(doc):
    is_valid = Chem.MolFromSmiles(doc)
    if is_valid:
        return f"{VALID_PREFIX} {doc}{EOS_TOKEN}"
...
def run(config):
    ...
    processed_data = process_docs(dataset)
    ...
    batched_data = concatenate_samples_without_splitting(
        processed_data, tokenizer, config.context_length
    )
