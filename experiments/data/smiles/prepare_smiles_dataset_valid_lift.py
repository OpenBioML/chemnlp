...
def process_docs(docs):
    valid = map(_get_valid_smile_string, docs)
    valid = [doc for doc in valid if doc is not None] # remove Nones
    valid = [string for pair in valid for string in pair] # unpack tuples
    
    invalid = map(_get_invalid_smile_string, docs)
    invalid = [doc for doc in invalid if doc is not None] # remove Nones

    mixed_data = valid + invalid
    return random.choice(mixed_data, len(mixed_data)).tolist()


def _get_invalid_smile_string(doc):
...
def _get_valid_smile_string(doc):
    ...
    if is_valid:
        return (
            f"{VALID_PREFIX} {doc}{EOS_TOKEN}",
            f"{LIFT_Q} {doc}? Answer: {VALID_LIFT_A}{EOS_TOKEN}"
        )
